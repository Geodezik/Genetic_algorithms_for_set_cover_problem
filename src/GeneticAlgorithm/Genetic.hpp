...
protected:
    ...
    BooleanMatrix::BooleanMatrix M = BooleanMatrix::BooleanMatrix(0, 0);
    int m;
    int n;
...
public:
    ...
    virtual double fitness(Individual& individual) = 0;
    ...
    void print_fit_stats(std::string filename="results.txt");
};
...
public:
    ...
    std::vector<bool> get_covered_rows(std::vector<bool> columns);
    int get_maxscore_column(std::vector<bool>& covered_rows, std::vector<bool>& columns, int row);
